...
async def get_latest_news():
    """Получает последние новости с выбранного сайта"""
    return await get_bbc_news()


async def get_bbc_news():
    """Парсит последнюю новость с BBC News"""
    try:
        ...
        main_news = soup.find('a', class_='gs-c-promo-heading')
        if not main_news:
            main_news = soup.find('h3', class_='gs-c-promo-heading__title')
        ...
        if main_news:
            ...
            return f"📰 **BBC News - Последняя новость:**\n\n{title}\n\n🔗 {link}"
        else:
            return "❌ Не удалось найти свежие новости на BBC News"
    except Exception as e:
        return f"❌ Ошибка при получении новостей: {str(e)}"


@router.callback_query(F.data == "news")
async def music_handler(callback: CallbackQuery):
    ...
    try:
        ...
        if callback.message.caption is not None:
            await callback.message.edit_caption(
                caption="❌ Не удалось получить новости",
                reply_markup=menu_button
            )
        else:
            await callback.message.edit_text(
